...
export class BoardsController {
  ...
  @Patch(':id')
  async update(@Param('id', ParseIntPipe) id: number, @Body() updateBoardDto: UpdateBoardDto, @Request() req) {
  ...
  @Post(':id/members')
  async addMember(@Param('id', ParseIntPipe) id: number, @Body() addMemberDto: AddMemberDto, @Request() req) {
    return this.boardsService.addMember(id, addMemberDto, req.user.userId);
  }

  @Delete(':id/members/:userId')
  async removeMember(@Param('id', ParseIntPipe) id: number, @Param('userId', ParseIntPipe) userId: number, @Request() req) {
    return this.boardsService.removeMember(id, userId, req.user.userId);
  }

  @Get(':id/members')
  async getMembers(@Param('id', ParseIntPipe) id: number, @Request() req) {
